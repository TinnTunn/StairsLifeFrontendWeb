    ...
    const list = roles.map(r => {
      ...
      <div class="card card-p-md" style="margin-bottom:8px;border:1px solid var(--border)">
        <div style="display:flex;align-items:center;justify-content:space-between;gap:8px">
          <div style="font-size:14px;font-weight:700">${esc(r.name)}${r.is_system ? ' <span class="badge badge-gray" style="font-size:9px">SYSTEM</span>' : ''}</div>
          ${r.is_system ? '' : `<button class="btn btn-ghost btn-sm" title="Hapus role" onclick="adminDeleteRole('${escAttr(r.id)}','${escAttr(r.name)}')">🗑</button>`}
        </div>
        ${r.description ? `<div style="font-size:12px;color:var(--text-secondary);margin-top:2px">${esc(r.description)}</div>` : ''}
        <div style="font-size:12px;color:var(--text-secondary);margin-top:4px">Anggota: ${esc(members.join(', ') || '-')}</div>
        <div style="font-size:12px;color:var(--text-muted);margin-top:6px">${esc(perms.join(' • ') || '-')}</div>
      </div>`;
    }).join('');
    const addForm = `
      <div class="card card-p-md" style="border:1px dashed var(--border);margin-top:4px">
        <div style="font-size:13px;font-weight:700;margin-bottom:8px">+ Tambah Role</div>
        <input class="form-input" id="new-role-name" placeholder="Nama role (mis. Finance Admin)" style="margin-bottom:8px">
        <input class="form-input" id="new-role-perms" placeholder="Permission, pisah koma (Overview,Finance)" style="margin-bottom:8px">
