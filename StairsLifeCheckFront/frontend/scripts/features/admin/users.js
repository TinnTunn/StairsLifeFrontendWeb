...
async function renderAdminUsers(tab) {
  ...
  try {
    ...
    el.innerHTML = users.map(u => {
      ...
      // SECURITY: u.full_name, u.email berasal dari user yang DAFTAR.
      // Admin view → tinggi prioritas-nya karena admin sering jadi target.
      const uidAttr   = escAttr(u.id);
      const nameSafe  = esc(u.full_name || '-');
      const nameAttr  = escAttr(u.full_name || '');
      const emailSafe = esc(u.email || '');
      const roleAttr  = escAttr(u.role || '');
      ...
      <div class="card card-p-md" style="margin-bottom:10px;display:flex;align-items:center;gap:14px">
        <div style="width:44px;height:44px;border-radius:50%;background:${isBisnis ? 'var(--teal-light)' : 'var(--accent-light)'};display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:700;color:${isBisnis ? 'var(--teal-dark)' : 'var(--accent)'};flex-shrink:0">
          ${esc((u.full_name || 'U').charAt(0))}
        </div>
        <div style="flex:1;min-width:0">
          <div style="font-size:14px;font-weight:600">${nameSafe}</div>
          <div style="font-size:12px;color:var(--text-secondary);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">
            ${emailSafe} · ${isBisnis ? 'Pemilik Bisnis' : 'Mahasiswa'}
          </div>
        </div>
        ${statusBadge}
        <button class="btn btn-ghost btn-sm" onclick="adminToggleSuspendUser('${uidAttr}', ${!!isSuspended})">
          ${isSuspended ? '🔓 Aktifkan' : '🚫 Suspend'}
        </button>
        <button class="btn btn-danger btn-sm" onclick="adminDeleteUser('${uidAttr}','${nameAttr}','${roleAttr}')">
          🗑️
        </button>
      </div>`;
    }).join('');
    ...
    el.innerHTML = `<div class="empty-state"><div class="empty-state-icon">❌</div><div class="empty-state-title">Gagal memuat pengguna</div><p class="empty-state-desc">${esc(e.message)}</p></div>`;
  }
}
...
// Deklarasi di sini supaya tersedia untuk adminToggleSuspendUser
// DAN _confirmSuspendUser (keduanya di bawah).
// BUG FIX: const tidak di-hoist — kalau deklarasi ada di bawah function
// yang memakainya, browser throw ReferenceError saat suspend diklik.
const _adminSuspendInFlight = new Set();
...
async function adminToggleSuspendUser(userId, isSuspended) {
  ...
  if (_adminSuspendInFlight.has(userId)) return;
  _adminSuspendInFlight.add(userId);
  try {
    ...
    _adminSuspendInFlight.delete(userId);
  }
}
...
const _adminDeleteUserInFlight = new Set();
async function _confirmAdminDeleteUser(userId, userRole) {
  if (_adminDeleteUserInFlight.has(userId)) return;
  ...
  _adminDeleteUserInFlight.add(userId);
  ...
  try {
    ...
    _adminDeleteUserInFlight.delete(userId);
  }
}
