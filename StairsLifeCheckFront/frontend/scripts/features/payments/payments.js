    ...
    <div class="modal-sheet" style="max-width:520px;overflow-y:auto;max-height:90vh" role="dialog">
      ...
      <div style="display:grid;gap:14px;margin-bottom:20px">
        <div class="form-group">
          ...
          <input class="form-input" id="escrow-amount" type="number" value="${amount}" placeholder="Sesuai kesepakatan" min="10000" max="100000000" step="1000" onblur="this.value=this.value?Math.round(this.value/1000)*1000:''">
...
async function submitEscrowAndContract(applicationId) {
  ...
  const amountRaw = parseInt(document.getElementById('escrow-amount')?.value || '0');
  ...
  try {
    ...
    //    Coba tab baru (state SPA tetap hidup). Kalau diblokir popup-blocker
    //    — umum terjadi karena open() tidak lagi dalam user-gesture langsung
    //    setelah beberapa await — fallback ke redirect tab yang sama.
    const win = window.open(invoiceUrl, '_blank', 'noopener');
  ...
  el.innerHTML = `
    <div style="max-width:480px;margin:40px auto;text-align:center;padding:24px">
      ...
      <div style="display:flex;gap:10px;justify-content:center">
        <button class="btn btn-primary" onclick="window.location.href = '/'">Kembali ke Dashboard</button>
        ${isSuccess ? '' : `<button class="btn btn-ghost" onclick="window.location.href = '#contracts'">Lihat Kontrak</button>`}
      </div>
    </div>
  `;
...
// Kembali ke app (dashboard) dari halaman standalone /payment/result.
// Kontrak ada di dalam dashboard bisnis; landing di '/' paling aman karena
// app melakukan routing pasca-login sendiri.
