...
async function onEnterStudent() {
  try {
    ...
    // Stat tiles (bento) — tulis lewat ID stabil (bukan lagi label-matching .stat-card)
    const setStat = (id, v) => { const el = document.getElementById(id); if (el) el.textContent = v; };
    setStat('st-stat-projects', user.total_projects || 0);
    setStat('st-stat-rating', user.rating_avg ? parseFloat(user.rating_avg).toFixed(1) : '0.0');
    setStat('st-stat-earnings', 'Memuat...');
