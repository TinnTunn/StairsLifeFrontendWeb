    ...
    el.innerHTML = disputes.map(d => `
      <div class="dispute-card">
        <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:10px">
          <span class="dispute-title">${d.project || 'Project'}</span>
          ${statusBadge(d.status)}
        </div>
        <div class="dispute-row"><svg viewBox="0 0 24 24"><line x1="12" y1="1" x2="12" y2="23"/></svg><span>Nilai: ${d.amount ? 'Rp ' + d.amount.toLocaleString('id-ID') : '-'}</span></div>
        <div class="dispute-row"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/></svg><span>${d.reason || '-'}</span></div>
        <div style="display:flex;gap:8px;margin-top:12px">
          <button class="btn btn-ghost btn-sm" onclick="openDisputeMediation('${d.id}')">💬 Chat Mediasi</button>
        </div>
      </div>`).join('');
  } catch (e) {
