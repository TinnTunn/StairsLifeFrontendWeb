...
async function loadContractDetail(contractId = null) {
  ...
  try {
    ...
    // Status pembayaran/escrow — HANYA untuk bisnis (payer) pada kontrak yang
    // belum selesai. Dipakai untuk menampilkan tombol "Bayar Sekarang" kalau
    // invoice belum dibayar / kedaluwarsa (sebelumnya tidak ada jalan re-pay
    // dari halaman kontrak, padahal pesan hasil pembayaran menyuruh ke sini).
    let payment = null;
    if (!isStudent && contract.status !== 'completed' && contract.status !== 'disputed') {
      try {
        const pRes = await PaymentsAPI.getByContract(contract.id);
        payment = pRes.data || null;
      } catch (_) {}
    }
    const escrowFunded = !!payment &&
      ['held', 'released', 'split_settled', 'refunded'].includes(String(payment.status));
    const needsPayment = !isStudent &&
      (contract.status === 'active' || contract.status === 'pending_review') &&
      !escrowFunded;
    ...
    if (statusBar) {
      statusBar.innerHTML = `
        ...
        ${needsPayment ? `
        <div style="background:var(--rose-light);border:1px solid rgba(244,63,94,0.25);border-radius:var(--radius-md);padding:16px;margin-bottom:16px">
          <div style="font-size:14px;font-weight:700;color:var(--rose);margin-bottom:4px">⚠️ Escrow belum didanai</div>
          <div style="font-size:13px;color:var(--text-secondary);margin-bottom:12px;line-height:1.6">
            ${payment && String(payment.status) === 'expired'
              ? 'Invoice pembayaran sebelumnya kedaluwarsa. Buat ulang untuk mengaktifkan proteksi escrow.'
              : 'Selesaikan pembayaran agar dana masuk escrow & mahasiswa bisa mulai mengerjakan project.'}
          </div>
          <button class="btn btn-primary btn-full" style="height:46px;font-size:15px"
            onclick="payContractInvoice('${escAttr(contract.id)}', ${Number(contract.agreed_budget) || 0})">
            🔒 Bayar Sekarang · Rp ${(Number(contract.agreed_budget) || 0).toLocaleString('id-ID')}
          </button>
        </div>` : ''}
        ...
        <div style="background:var(--teal-light);border:1px solid rgba(31,185,125,0.3);border-radius:var(--radius-md);padding:14px;margin-bottom:16px">
        ...
        <div style="background:var(--teal-light);border:1px solid rgba(31,185,125,0.3);border-radius:var(--radius-md);padding:14px;margin-bottom:16px">
          ...
          <div style="display:flex;flex-wrap:wrap;gap:5px;margin-bottom:8px">
            ${(existingReview.tags || []).map(t => `
              <span style="background:rgba(31,185,125,0.2);color:var(--teal-dark);font-size:11px;padding:3px 8px;border-radius:12px;font-weight:600">${esc(t)}</span>
            `).join('')}
          </div>` : ''}
      ...
      <div style="padding: 20px 24px 24px">
        ...
        <div style="display:flex;gap:10px">
          ...
          <button class="btn btn-primary" style="
              ...
              font-weight: 700;
              border-radius: 12px;
              box-shadow: 0 4px 12px rgba(91, 91, 245, 0.28);
