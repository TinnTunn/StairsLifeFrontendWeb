...
async function loadFinanceDetail() {
  ...
  try {
            ...
            ${payments.map(p => {
              ...
              const badge    = p.status === 'released'
                ...
                : p.status === 'held'
                  ...
                  : `<span class="badge badge-gray" style="font-size:10px">${p.status}</span>`;
              return `
                <tr style="border-bottom:1px solid var(--divider)"
                    onmouseover="this.style.background='var(--bg-surface)'"
                    onmouseout="this.style.background=''">
                  <td style="padding:10px 8px;font-weight:600;max-width:160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${project.title || ''}">${project.title || '-'}</td>
                  <td style="padding:10px 8px;color:var(--text-secondary)">${biz.full_name || '-'}</td>
                  <td style="padding:10px 8px;color:var(--text-secondary)">${student.full_name || '-'}</td>
                  ...
                  <td style="padding:10px 8px;color:var(--text-muted)">${fmtDate(p.released_at || p.created_at)}</td>
                </tr>`;
            }).join('')}
          </tbody>
        </table>
    ...
    tableEl.innerHTML = `<div class="empty-state"><div class="empty-state-icon">❌</div><div class="empty-state-title">Gagal memuat transaksi</div><p class="empty-state-desc">${e.message}</p></div>`;
  }
}
