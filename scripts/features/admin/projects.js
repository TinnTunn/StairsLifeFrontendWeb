...
async function renderAdminRecentProjects() {
  ...
  try {
    ...
    el.innerHTML = `<div class="card" style="overflow:hidden">
      ...
      ${projects.map(p => `
      <div class="admin-project-row" style="cursor:pointer" onclick="adminOpenProjectDetail('${escAttr(p.id)}')">
        <div style="font-weight:500;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(p.title)}</div>
        <div style="font-size:13px;color:var(--text-secondary);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(p.users?.full_name || '-')}</div>
...
async function renderAdminProjectManagement() {
  ...
  try {
    ...
    // Urutkan berdasarkan tanggal dibuat sesuai toggle Newest/Oldest.
    projects = projects.slice().sort((a, b) => {
      const ta = new Date(a.created_at || 0).getTime();
      const tb = new Date(b.created_at || 0).getTime();
      return adminProjectView.sort === 'oldest' ? ta - tb : tb - ta;
    });
    ...
    el.innerHTML = `<div class="card" style="overflow:hidden">
      ...
      <div class="admin-project-row" style="cursor:pointer"
        onclick="adminOpenProjectDetail('${escAttr(p.id)}')"
        onmouseover="this.style.background='var(--bg-surface)'"
        onmouseout="this.style.background=''">
        <div style="font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(p.title)}</div>
        <div style="font-size:13px;color:var(--text-secondary);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(p.users?.full_name || '-')}</div>
        <div>${statusBadge(p.status === 'inProgress' ? 'aktif' : p.status === 'completed' ? 'selesai' : 'open')}</div>
...
async function adminOpenProjectDetail(projectId) {
  ...
  try {
    ...
    detailEl.innerHTML = `
      <h2 style="font-size:18px;font-weight:800;margin-bottom:4px">${esc(project.title)}</h2>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:16px">
        <div class="card card-p-md">
          ...
          <div style="font-weight:600">${esc(project.users?.full_name || '-')}</div>
          <div style="font-size:12px;color:var(--text-secondary)">${esc(project.users?.email || '')}</div>
        </div>
        <div class="card card-p-md">
          ...
          <div style="font-size:12px;color:var(--text-secondary)">Deadline: ${esc(fmtDate(project.deadline))}</div>
        </div>
      </div>
      ...
      <div class="card card-p-md" style="margin-bottom:16px">
        ...
        <div style="font-size:13px;color:var(--text-secondary);line-height:1.6;white-space:pre-wrap">${esc(project.description || '-')}</div>
      </div>
    ...
    detailEl.innerHTML = `<div class="empty-state"><div class="empty-state-icon">❌</div><div class="empty-state-title">${esc(e.message)}</div></div>`;
  }
}
function _parseDelivUrls(urlField) {
  if (!urlField) return [];
...
async function adminOpenDeliverableFile(urlOrPath) {
  try {
  ...
  return `
    <div class="card card-p-md" style="margin-bottom:12px">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;flex-wrap:wrap;gap:8px">
        <div>
          <div style="font-size:13px;font-weight:700">Mahasiswa: ${esc(student.full_name || '-')}</div>
          <div style="font-size:12px;color:var(--text-secondary)">${esc(student.email || '')}</div>
        </div>
      ...
      ` : delivs.map((d, i) => `
        <div style="border:1px solid var(--border);border-radius:var(--radius-md);padding:10px;margin-bottom:8px">
          ...
          ${d.deliverable_notes ? `
            <div style="font-size:12px;color:var(--text-secondary);margin-bottom:6px;line-height:1.5;white-space:pre-wrap">${esc(d.deliverable_notes)}</div>
          ` : ''}
          ${d.deliverable_url ? `
            <div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:6px">
              ${_parseDelivUrls(d.deliverable_url).map((urlOrPath, idx) => {
                  const parts = urlOrPath.split('/');
                const rawName = decodeURIComponent(parts[parts.length - 1] || `File ${idx + 1}`);
                  const displayName = rawName.length > 30 ? rawName.substring(0, 27) + '...' : rawName;
                return `<button type="button" class="btn btn-ghost btn-sm"
                  style="font-size:11px;display:inline-flex;align-items:center;gap:5px"
                  onclick="adminOpenDeliverableFile('${escAttr(urlOrPath)}')">
                  📎 ${esc(displayName)}
                  </button>`;
              }).join('')}
              </div>
          ...
          ${d.rejection_reason ? `
            <div style="font-size:12px;color:var(--rose);background:var(--rose-light);padding:6px 8px;border-radius:var(--radius-sm)">
              ❌ Alasan ditolak: ${esc(d.rejection_reason)}
            </div>
          ` : ''}
          <div style="font-size:11px;color:var(--text-muted);margin-top:6px">
            Submit: ${esc(fmtDate(d.submitted_at))}
            ${d.reviewed_at ? ` · Review: ${esc(fmtDate(d.reviewed_at))}` : ''}
          </div>
        </div>
      `).join('')}
    </div>`;
...
window.adminOpenDeliverableFile = adminOpenDeliverableFile;