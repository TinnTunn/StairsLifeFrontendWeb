...
async function renderAdminSupportInbox() {
  ...
  try {
    ...
    listEl.innerHTML = `<div class="empty-state"><div class="empty-state-icon">❌</div><div class="empty-state-title">Gagal memuat</div><p class="empty-state-desc">${e.message}</p></div>`;
  }
}
...
function renderSupportUserCard(u) {
  ...
  <div class="card card-p-md" style="margin-bottom:10px;display:flex;align-items:flex-start;gap:14px;${suspended ? 'border-left:3px solid var(--rose)' : ''}">
    <div style="width:44px;height:44px;border-radius:50%;background:${suspended ? 'var(--rose-light,#fef2f2)' : 'var(--accent-light)'};display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:800;color:${suspended ? 'var(--rose)' : 'var(--accent)'};flex-shrink:0">${avatar}</div>
    <div style="flex:1;min-width:0">
      ...
      <div style="font-size:12px;color:var(--text-secondary);margin-bottom:2px">${u.email || ''}</div>
      ...
      <div style="font-size:11px;color:var(--rose);padding:4px 8px;background:var(--rose-light,#fef2f2);border-radius:6px;margin-top:4px;display:inline-block">
        ⚠️ Alasan: ${u.suspension_reason}
      </div>` : ''}
    </div>
    <div style="display:flex;flex-direction:column;gap:6px;flex-shrink:0">
      <button class="btn btn-ghost btn-sm"
        onclick="openAdminSupportChat('${roomId}', '${name.replace(/'/g, "\\'")}')">
        💬 Lihat Chat
      </button>
      ${suspended ? `
      <button class="btn btn-primary btn-sm" id="unsuspend-btn-${u.id}"
        onclick="adminUnsuspendUser('${u.id}', this)">
        🔓 Aktifkan
      </button>` : `
      <button class="btn btn-danger btn-sm"
        onclick="adminToggleSuspendUser('${u.id}', false)">
        🚫 Suspend
      </button>`}
    </div>
  </div>`;
      ...
      if (!messages.length) {
        msgsEl.innerHTML = `
          <div style="text-align:center;padding:40px 20px">
            ...
            <div style="font-size:14px;color:var(--text-secondary)">Belum ada pesan dari ${userName}</div>
          </div>`;
      } else {
          ...
          return `
            <div class="msg-row" style="align-items:${isAdmin ? 'flex-end' : 'flex-start'}">
              <div class="msg-bubble${isAdmin ? ' me' : ' them'}">
                ${!isAdmin ? `<div style="font-size:11px;font-weight:700;opacity:0.7;margin-bottom:4px">${name}</div>` : ''}
                ${m.content}
              </div>
              <div class="msg-time${isAdmin ? '' : ' them'}">${time}</div>
            </div>`;
        }).join('');
...
async function adminResolveSupport(roomId) {
  showToast('Tiket ditandai resolved ✅', 'success');
  await renderAdminSupportInbox();
}
