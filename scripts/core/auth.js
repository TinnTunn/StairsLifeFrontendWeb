...
function switchLoginRole(i) {
  ...
  // Update kolom art kiri (aurora) sesuai role — design system Auth split-layout
  const ART = [
    { title: 'Mulai naik <span class="accent">tangga karir</span>. Project demi project.',
      desc: 'Setiap project selesai = satu anak tangga. Verifikasi KTM, escrow payment, dan portofolio otomatis yang ikut kamu lulus.',
      quote: '"Saya dapat tiga klien tetap dari StairsLife. Sekarang punya cukup buat bayar UKT sendiri."',
      who: 'Dina, Semester 5', whoSub: 'Universitas Brawijaya · 12 project', av: 'D' },
    { title: 'Temukan <span class="accent">talenta mahasiswa</span> untuk bisnismu.',
      desc: 'Pasang project, terima lamaran mahasiswa terverifikasi, dan bayar aman lewat escrow. Hasil kerja dijamin lewat kontrak digital.',
      quote: '"Feed Instagram toko saya jauh lebih rapi sejak pakai StairsLife. Talentanya muda, cepat, dan terjangkau."',
      who: 'Kue Artisan Nusantara', whoSub: 'UMKM Kuliner · 8 project selesai', av: 'K' },
    { title: 'Jaga <span class="accent">ekosistem</span> tetap aman & terpercaya.',
      desc: 'Kelola verifikasi KTM, mediasi sengketa, dan pantau arus dana escrow dari satu dashboard operasional.',
      quote: '"Panel admin yang rapi bikin proses verifikasi dan mediasi jadi cepat dan transparan."',
      who: 'Tim Operasional', whoSub: 'StairsLife · Internal', av: 'T' },
  ];
  const a = ART[i] || ART[0];
  const set = (id, html, isHtml) => { const el = document.getElementById(id); if (el) { if (isHtml) el.innerHTML = html; else el.textContent = html; } };
  set('auth-art-title', a.title, true);
  set('auth-art-desc', a.desc, false);
  set('auth-art-quote', a.quote, false);
  set('auth-art-who', a.who, false);
  set('auth-art-whosub', a.whoSub, false);
  set('auth-art-who-av', a.av, false);
}
...
async function handleLogin() {
  ...
  try {
    ...
    if (user.is_suspended) {
      showSuspendedModal(user.suspension_reason || 'Melanggar ketentuan penggunaan platform.', user.email || email);
      return;
    }
    ...
    try {
      ...
      if (parsed.suspended) {
        showSuspendedModal(parsed.reason || 'Melanggar ketentuan penggunaan platform.', email);
        return;
      }
    } catch (_) {}
    ...
    if (raw.toLowerCase().includes('suspended') ||
        ...
        raw.toLowerCase().includes('disuspend')) {
      // Backend kirim pesan teks (bukan JSON) — tetap tampilkan modal banding
      showSuspendedModal('Akun kamu telah dinonaktifkan sementara oleh admin.', email);
      return;
  ...
  return `
    <div class="card card-p-lg" style="text-align:center">
      ...
      <button class="btn btn-primary btn-full" style="height:48px;font-size:15px" id="reg-finish-btn" onclick="finishRegistration()">
        ...
        <svg class="btn-loader" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.5" width="20"><circle cx="12" cy="12" r="10"/></svg>
      </button>
    </div>`;
}
...
function _confirmLogout() {
  ...
  // FULL RELOAD ke root (bukan goTo SPA). Ini membersihkan SEMUA state
  // in-memory (cache project/wallet/chat, dll) + reset DOM (tab kembali ke
  // home). Tujuannya: login berikutnya selalu mulai BERSIH di dashboard,
  // tidak peduli path/tab terakhir & tidak ada data akun sebelumnya nyangkut.
 ...
 */
function showSuspendedModal(reason, email) {
  const safeReason = (window.esc ? esc(reason) : reason) || 'Melanggar ketentuan penggunaan platform.';
  ...
  modal.style.zIndex = '9999';
  modal.innerHTML = `
    <div class="modal-sheet" style="max-width:440px;text-align:center" role="alertdialog"
         aria-modal="true" aria-labelledby="susp-title" aria-describedby="susp-desc">
      <div class="susp-icon" aria-hidden="true">
        <svg viewBox="0 0 24 24" width="30" height="30" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="4.93" y1="4.93" x2="19.07" y2="19.07"/></svg>
      </div>
      <h2 id="susp-title" style="font-size:19px;font-weight:800;color:var(--rose);margin-bottom:8px">Akun Kamu Disuspend</h2>
      <p id="susp-desc" style="font-size:14px;color:var(--text-secondary);line-height:1.6;margin-bottom:12px">Untuk sementara kamu tidak bisa mengakses fitur. Alasan dari admin:</p>
      <div class="susp-reason" style="border-radius:var(--radius-md);padding:12px 14px;margin-bottom:16px;text-align:left;display:flex;gap:10px;align-items:flex-start">
        <span aria-hidden="true" style="flex-shrink:0">⚠️</span>
        <p style="font-size:14px;font-weight:600;margin:0;line-height:1.5">${safeReason}</p>
      </div>
      <p style="font-size:13px;color:var(--text-muted);margin-bottom:18px">Merasa ini keliru? Hubungi admin lewat chat langsung untuk mengajukan banding.</p>
      <div style="display:flex;flex-direction:column;gap:10px;margin-bottom:8px">
        <button class="btn btn-primary btn-full" style="height:46px" onclick="_contactAdminFromSuspend()">
          <svg viewBox="0 0 24 24" width="17" height="17" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z"/></svg>
          Hubungi Admin (Chat)
        </button>
      </div>
      <button class="btn btn-ghost btn-full" style="margin-top:6px" onclick="_closeSuspendedModal()">Tutup</button>
    </div>`;
  document.body.appendChild(modal);

  // A11y: fokus ke modal + Esc untuk menutup + klik backdrop menutup
  const sheet = modal.querySelector('.modal-sheet');
  sheet?.setAttribute('tabindex', '-1');
  setTimeout(() => sheet?.focus(), 30);
  modal.addEventListener('mousedown', (e) => { if (e.target === modal) _closeSuspendedModal(); });
  modal._keyHandler = (e) => { if (e.key === 'Escape') _closeSuspendedModal(); };
  document.addEventListener('keydown', modal._keyHandler);
}

/**
 * Tutup modal suspend lalu BUKA CHAT SUPPORT real-time ke admin.
 * Sesi (token) TIDAK di-clear di sini — chat butuh sesi yang sama.
 * Di sisi admin, nama mahasiswa/bisnis muncul dengan label "🚫 Suspended"
 * di panel Support (lihat features/admin/support.js).
 */
function _contactAdminFromSuspend() {
  const modal = document.getElementById('suspended-modal');
  if (modal && modal._keyHandler) document.removeEventListener('keydown', modal._keyHandler);
  modal?.remove();
  // Tandai sesi sebagai "suspended-contact" supaya app hanya membuka chat
  try { sessionStorage.setItem('sl_suspended_contact', '1'); } catch (_) {}
  if (typeof openSupportChat === 'function') {
    openSupportChat();
  } else {
    // Fallback: ke daftar chat
    if (typeof goTo === 'function') goTo('screen-chat-list');
  }
}
...
function _closeSuspendedModal() {
  ...
  if (modal && modal._keyHandler) document.removeEventListener('keydown', modal._keyHandler);
  modal?.remove();
  TokenManager.clear(); // Clear token saat user tutup modal tanpa banding
}
...
window._contactAdminFromSuspend = _contactAdminFromSuspend;
