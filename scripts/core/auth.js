...
async function handleKtmFileChange(event) {
  ...
  // Simpan file object — upload dilakukan setelah register & dapat token
  REG.ktmFile     = file;
...
async function finishRegistration() {
  ...
  try {
    ...
    // Upload KTM lalu submit verifikasi setelah punya token
    if (role === 'mahasiswa' && REG.ktmFile && !state.regSkipKtm) {
      try {
        const uploadRes = await UploadAPI.uploadFile(REG.ktmFile, 'ktm');
        REG.ktmUrl = uploadRes?.data?.url || '';
        if (REG.ktmUrl) {
